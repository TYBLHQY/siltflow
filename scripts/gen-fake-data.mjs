...
class Rng {
  ...
  approx(val, spread = 0.15) {
    return val * (1 + (this.next() - 0.5) * 2 * spread);
  }
}
...
const annLines = readFileSync("/tmp/existing-annotations.txt", "utf-8")
  .trim()
  .split("\n");
...
for (let i = 0; i < cardAnnotations.length; i++) {
  ...
  if (state === 0) {
    ...
    difficulty =
      Math.round(
        Math.min(
          Math.max(rng.approx(0.3 + rng.next() * 0.6, 0.1), 0.15),
          0.95,
        ) * 100,
      ) / 100;
  ...
  const cardData = {
    ...
    stability,
    difficulty,
    elapsed_days: state === 0 ? 0 : rng.int(0, 30),
    scheduled_days,
    reps,
    lapses,
    state,
  ...
  cards.push({
    ...
    createdAt: isoTs(createdDate),
    updatedAt: isoTs(createdDate),
  });
}
...
for (const card of cards) {
  ...
  for (let r = 0; r < numReviews; r++) {
    ...
    currDiff = Math.min(
      Math.max(currDiff + (grade >= 3 ? -0.02 : 0.05), 0.1),
      0.9,
    );
    ...
    const logData = {
      ...
      log: {
        ...
        stability: currStab,
        difficulty: currDiff,
        scheduled_days: Math.round(currStab),
        review: reviewTs,
      },
      card: {
        due: reviewTs,
...
const insertManyLogs = db.transaction((items) => {
  for (const l of items)
    insertLog.run(l.id, l.annotationId, l.documentId, l.data, l.createdAt);
});
insertManyLogs(logs);
...
const dd = db
  .prepare(
    "SELECT DISTINCT substr(created_at,1,10) as d FROM review_logs ORDER BY d",
  )
  .all();
const ss = db
  .prepare(
    "SELECT json_extract(data,'$.state') as s, count(*) as c FROM fsrs_cards GROUP BY s ORDER BY s",
  )
  .all();
db.close();

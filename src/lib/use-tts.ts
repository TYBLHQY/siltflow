...
function buildMimoBody(config: ReturnType<typeof useTTSStore.getState>["config"], voice: string, text: string) {
    ...
    if (config.provider === "mimo") {
      ...
      try {
        const response = await fetch(MIMO_ENDPOINT, {
          ...
          body: JSON.stringify(buildMimoBody(config, voice || config.mimoVoice, text)),
        })

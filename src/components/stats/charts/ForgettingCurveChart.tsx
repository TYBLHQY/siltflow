...
const LINE_COLORS = [
  "var(--catppuccin-color-mauve)", "var(--catppuccin-color-blue)",
  "var(--catppuccin-color-green)", "var(--catppuccin-color-peach)",
...
export function ForgettingCurveChart() {
  ...
  return (
    <ChartCard
      title="Forgetting Curves" loading={loading} isEmpty={isEmpty} emptyMessage="No reviewed cards yet"
    >
      <ResponsiveContainer width="100%" height={280}>
        <LineChart data={filtered} margin={{ top: 4, right: 4, left: -12, bottom: 0 }}>
          <ChartGrid />
          <XAxis dataKey="day" tick={{ fontSize: 10 }} stroke="var(--catppuccin-color-text)"
            label={{ value: "Days elapsed", position: "insideBottomRight", offset: -4, fontSize: 10, fill: "var(--catppuccin-color-text)" }} />
              ...
              <div className="mt-1 flex flex-wrap justify-center gap-x-4 gap-y-1 text-xs">
                {FORGETTING_LABELS.map((lbl, idx) => (
                  <div key={lbl} className="flex items-center gap-1">
                    <span className="inline-block h-0.5 w-3 rounded-full" style={{ backgroundColor: LINE_COLORS[idx] }} />
        ...
        {[30, 90, 180, 365].map((d) => (
          <button
            ...
            className={`rounded px-2 py-0.5 text-xs font-medium transition-colors ${
              maxDays === d ? "bg-ctp-mauve text-ctp-crust" : "bg-ctp-surface0 text-ctp-text hover:bg-ctp-surface0"
            }`}
            onClick={() => setMaxDays(d)}>{d}d</button>
        ))}
      </div>
    </ChartCard>
  );

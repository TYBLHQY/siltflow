...
export function KnowledgeGrowthChart() {
  ...
  const data = useMemo(() => computeKnowledgeGrowth(logs), [logs]);
      ...
      <ResponsiveContainer width="100%" height={280}>
        ...
        >
          ...
          <Tooltip
            ...
            itemSorter={(item: any) =>
              MATURITY_ORDER[item.name as string] ?? 99
            }
          />
          <Legend
            ...
            content={({ payload }) => {
              const sorted = payload
                ? [...payload].sort(
                    (a, b) =>
                      (MATURITY_ORDER[a.value as string] ?? 99) -
                      (MATURITY_ORDER[b.value as string] ?? 99),
                  )
                : [];
              return (
                  ...
                  {sorted.map((entry) => (
                    <div key={entry.value} className="flex items-center gap-1">
                      <span
                        ...
                        style={{
                          backgroundColor:
                            LEGEND_COLORS[entry.value as string] ??
          ...
          <Area
            ...
            name="Learning"
            stackId="1"
            stroke="var(--catppuccin-color-red)"
          ...
          <Area
            ...
            name="Young"
            stackId="1"
            stroke="var(--catppuccin-color-yellow)"
            fill="url(#gradYoung)"
            fillOpacity={1}
          />
          <Area
          ...
          <Area
            ...
            name="Long-term"
            stackId="1"
            stroke="var(--catppuccin-color-blue)"

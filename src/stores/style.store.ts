...
const DEFAULT_STYLE: ParagraphStyle = {
  ...
  globalFontSize: 14,
  pdfScrollbar: false,
  systemFontFamilies: ["system-ui", "-apple-system", "Segoe UI", "Roboto", "Helvetica", "Arial", "sans-serif"],
}
...
export const useStyleStore = create<StyleState>((set) => ({
  ...
  removeFontFamily: (index) =>
    set((s) => {
      const next = { ...s.style, fontFamilies: s.style.fontFamilies.filter((_, i) => i !== index) }
      persist(next)
      return { style: next }
    }),
  ...
  addSystemFontFamily: (family) =>
    set((s) => {
      const next = { ...s.style, systemFontFamilies: [...s.style.systemFontFamilies, family] }
      persist(next)
      return { style: next }
    }),
  ...
  removeSystemFontFamily: (index) =>
    set((s) => {
      const next = { ...s.style, systemFontFamilies: s.style.systemFontFamilies.filter((_, i) => i !== index) }
      persist(next)
      return { style: next }
    }),
    ...
    if (saved && typeof saved === "object") {
      ...
      useStyleStore.setState({
        style: {
          ...
          systemFontFamilies:
            (s.systemFontFamilies as string[]) ?? DEFAULT_STYLE.systemFontFamilies,
        },
      })
    }
  } catch { /* ignore */ }
